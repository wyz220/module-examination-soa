#!/bin/sh
#
# Start/Stop the retail water server.
#
...
waitForPid () {
  ...
  while [ 1 -eq 1 ]; do
    ...
    if [ $? -eq 1 ]; then
      echo "RW server PID ${MTRPID} exited"
      return 1
    fi
  ...
  echo "rw server PID ${MTRPID} did not exit."
  return 0;
}
...
doStopSignal () {
  ...
  if [ -f "${SERVER_PID}" ] ; then
    ...
    if [ "x${MTRPID}" = "x" ] ; then
      echo "rw server pid file was empty: ${SERVER_PID}"
      exit 127
    fi
    ...
    echo "rw server not running (no pid file found: ${SERVER_PID})"
  fi
}
...
if [ -f "${SERVER_PID}" ] ; then
  ...
  if [ ! "x${MTRPID}" = "x" ] ; then
    ...
    if [ $? -eq 1 ]; then
      ...
      echo "rw server is already running (pid ${MTRPID})."
      exit 0
    fi
  fi
fi
...
# Start the server
echo "Booting the rw server (Using JAVA_OPTS=${JAVA_OPTS})..."

java ${JAVA_OPTS} -cp ${CLASSPATH} com.msht.retailwater.RWServerApplication > ${SERVER_LOG} 2>&1 &
...
case "$1" in
  start)
    echo "Starting rw server..."
    doStart
    echo "rw server booted."
    ;;
  stop)
    echo "Stopping rw server..."
    doStop
    echo "rw server is stopped."
    ;;
  *)
